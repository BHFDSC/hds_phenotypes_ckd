...
# MAGIC
# MAGIC **Description** This notebook derives the last observable date for those who are alive as at the project pipeline end date. That is, it works out the date at which GDPPR and HES APC collectively have no coverage lag.
# MAGIC
...
# If user does not want to use last_observable_date then, pipeline_production_date is set to last_observable_date
if(last_observable_flag == 'False' and does_not_exist_toggle):
    data = [("gdppr", pipeline_production_date),
         ("hes_apc", pipeline_production_date),
         ("hes_apc_diagnostic", pipeline_production_date),
         ("all", pipeline_production_date)]
...
if(last_observable_flag == 'False' and does_not_exist_toggle):
    save_table(df=parameters_df_last_observable_date, 
               out_name=parameters_df_last_observable_date_name, save_previous=False)

# override the compute
if(last_observable_flag == 'False'):
    does_not_exist_toggle = False
