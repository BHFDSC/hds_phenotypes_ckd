...
# MAGIC %md
# MAGIC
# MAGIC # KDSC Parameters for Defining Chronic Kidney Disease (CKD) Algorithm
# MAGIC
# MAGIC **Parent Project** KDSC
# MAGIC
...
# MAGIC
# MAGIC **Acknowledgements** Based on ddsc-parameters, CCU003_05-D01-parameters and CCU002_07
# MAGIC
...
# Project
# -----------------------------------------------------------------------------
# e.g. ccu001_01 (you might also want to set this as a unique identifier specific to your study end date too eg. ccu000_2023)

dbutils.widgets.text("proj",
                     "kdsc_new", # DEFAULT VALUE e.g. kdsc_new
                     "Project Name")
proj = dbutils.widgets.get("proj")
print(proj)
...
# -----------------------------------------------------------------------------
# if True, the last observable date will be computed and records that occur after this date will be excluded, if False then this date is not implemented and the 'last_observable_date' is set to the pipeline_production_date
dbutils.widgets.text("last_observable_flag",
                     "True", # DEFAULT VALUE e.g. False
                     "Last Observable Date Flag")
last_observable_flag = dbutils.widgets.get("last_observable_flag")
print(last_observable_flag)
...
dbutils.widgets.text("pipeline_production_date",
                     "2025-07-15", # DEFAULT VALUE e.g. 2025-07-15
                     "Pipeline Production Date")
pipeline_production_date = dbutils.widgets.get("pipeline_production_date")
print(pipeline_production_date)
# Do note that last_observable_date can be generated to account for data lag in the run up to pipeline_production_date
...
# For example if you wanted to find the CKD status as at 2020-01-01 you must set your study end date to this, but pick a pipeline_production_date that is later than 2020-12-11
# note that if last_observable_date < study_end_date then then study_end_date will be set to last_observable_date

dbutils.widgets.text("study_end_date",
                     "2024-12-31", # DEFAULT VALUE e.g. 2024-12-31
                     "Study End Date")
study_end_date = dbutils.widgets.get("study_end_date")
...
# Study Start Date
# -----------------------------------------------------------------------------
# in the context of this algorithm, this is the date at which records before this date are not considered for inclusion. We recommend setting this to 1900-01-01 i.e. all records >=1900-01-01 will be extracted from the underlying data sources

dbutils.widgets.text("study_start_date",
                     "1900-01-01", # DEFAULT VALUE e.g. 1900-01-01
                     "Study Start Date")
study_start_date = dbutils.widgets.get("study_start_date")
print(study_start_date)

# -----------------------------------------------------------------------------
# Individual censor end dates (dates at which a persons CKD type will be classified)
# -----------------------------------------------------------------------------
# if you have individual censor end dates set this to True, else False
dbutils.widgets.text("individual_censor_dates_flag",
                     "False", # DEFAULT VALUE e.g. False
                     "Individual Censor Flag")
individual_censor_dates_flag = dbutils.widgets.get("individual_censor_dates_flag")
print(individual_censor_dates_flag)

# if individual_censor_dates_flag = True, set the db and table name for the censor dates e.g. as_test_ckd_censor_end. Your table should contain 2 columns, PERSON_ID and CENSOR_END. If individual_censor_dates_flag = False then set individual_censor_dates_table = None
dbutils.widgets.text("individual_censor_dates_table",
                     "dsa_391419_j3w9t_collab.as_test_ckd_censor_end", # DEFAULT VALUE e.g. dsa_391419_j3w9t_collab.kdsc_individual_censor_dates_table_empty
                     "Individual Censor Dates")
individual_censor_dates_table = dbutils.widgets.get("individual_censor_dates_table")
print(individual_censor_dates_table)
...
# MAGIC %md
# MAGIC ## 3.2 Algorithm Version and Timestamp
...
# MAGIC %md
# MAGIC # 5. HES APC procedure/operations table
...
df = (
    spark
        .sql("show tables in dsa_391419_j3w9t_collab")
        .select("tableName")
        .filter("tableName LIKE '%hds_curated_assets__hes_apc_procedure%'")
        .filter(~f.col('tableName').contains('pre'))
        .orderBy(f.col("tableName").desc())
        .withColumn("date_str", f.regexp_extract("tableName", r"(\d{4}_\d{2}_\d{2})", 1))
        .withColumn("date", f.to_date("date_str", "yyyy_MM_dd"))
...
path_hes_apc_proc = f'{dsa}.{df}'
print(f'HES APC procedure Table Version: ',path_hes_apc_proc)